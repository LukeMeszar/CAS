...
DefTableKey InputFileName = NoKey;
DefTableKey BaseFileName = NoKey;
static int BaseFileName_count = 0;
static int error_count = 0;

#define NUMOPTS 1

static opt opts[] = {
    { "-base", STRVAL | SPACED, &BaseFileName_count, &BaseFileName, NULL },
...
#endif
{
  ...
  fprintf (stderr, "    -base string Base file name for output\n");
  fprintf (stderr, "  Parameters:\n");
  fprintf (stderr, "    InputFileName Monitoring interface description\n");
    exit (1);
}
...
#endif
{
  ...
  if (i < argc) {
    InputFileName = NewKey ();
    SetClpValue (InputFileName, clp_string (argv[i++]), 0);
    CLP_InputFile = InputFileName;
  }
