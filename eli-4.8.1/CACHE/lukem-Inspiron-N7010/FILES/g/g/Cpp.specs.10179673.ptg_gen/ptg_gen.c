...
#endif
{
	...
	PTG_OUTPUT_STRING(f, n->p1);
	PTG_OUTPUT_STRING(f, " *>");
	PTG_OUTPUT_STRING(f, "(\" [BP_BeginBlockI] [BP_BreakLine] \"1,\" [BP_BreakLine] ");
	PTG_OUTPUT_STRING(f, "$ [BP_BreakLine] [BP_EndBlockI] \")\"\n");
}
...
#endif
{
	...
	PTG_OUTPUT_STRING(f, n->p1);
	PTG_OUTPUT_STRING(f, "_H\n");
	PTG_OUTPUT_STRING(f, "#define ");
	PTG_OUTPUT_STRING(f, n->p1);
	PTG_OUTPUT_STRING(f, "_H\n\n");
	PTG_OUTPUT_STRING(f, "#include <string>\n");
	PTG_OUTPUT_STRING(f, "#include <list>\n\n");
	n->p2->_print(n->p2);
	PTG_OUTPUT_STRING(f, "\nclass ");
	PTG_OUTPUT_STRING(f, n->p1);
	...
	PTG_OUTPUT_STRING(f, n->p1);
	PTG_OUTPUT_STRING(f, "() { }\n");
	PTG_OUTPUT_STRING(f, "    virtual ~");
	PTG_OUTPUT_STRING(f, n->p1);
	PTG_OUTPUT_STRING(f, "() { }\n");
	PTG_OUTPUT_STRING(f, "    virtual void Accept(");
	PTG_OUTPUT_STRING(f, n->p1);
	PTG_OUTPUT_STRING(f, "Visitor *) { }\n");
	PTG_OUTPUT_STRING(f, "};\n\n");
	n->p4->_print(n->p4);
...
#ifdef PROTO_OK
PTGNode PTGHeader(CONST char* p1, PTGNode p2, PTGNode p3, PTGNode p4, PTGNode p5, PTGNode p6)
#else
PTGNode PTGHeader(p1, p2, p3, p4, p5, p6)
...
#endif
{
	PTG_OUTPUT_STRING(f, "    virtual void Visit");
	PTG_OUTPUT_STRING(f, n->p1);
	PTG_OUTPUT_STRING(f, "(");
	PTG_OUTPUT_STRING(f, n->p1);
...
#endif
{
	PTG_OUTPUT_STRING(f, "    void SetChild");
	PTG_OUTPUT_INT(f, n->p2);
	PTG_OUTPUT_STRING(f, "(");
	n->p1->_print(n->p1);
...
/* Implementation of Pattern Vars */

typedef struct _SPTGVars{
	_PTGProc _print;
	PTGNode p1;
} * _PPTGVars;

#ifdef PROTO_OK
static void _PrPTGVars(_PPTGVars n)
#else
static void _PrPTGVars(n)
	_PPTGVars n;
#endif
{
...
{
	_PPTGVars n;
	n = (_PPTGVars)MALLOC(sizeof(struct _SPTGVars));
	n->_print = (_PTGProc)_PrPTGVars;
	n->p1 = p1;
	return (PTGNode)n;
}
...
#endif
{
	...
	PTG_OUTPUT_STRING(f, "    list<");
	PTG_OUTPUT_STRING(f, n->p3);
	PTG_OUTPUT_STRING(f, " *> *GetList() { return c; }\n");
	PTG_OUTPUT_STRING(f, "  private:\n");
	PTG_OUTPUT_STRING(f, "    list<");
	PTG_OUTPUT_STRING(f, n->p3);
	PTG_OUTPUT_STRING(f, " *> *c;\n");
