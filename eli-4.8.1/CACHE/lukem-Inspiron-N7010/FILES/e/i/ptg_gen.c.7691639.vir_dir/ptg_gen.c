...
#endif
{
	...
	PTG_OUTPUT_STRING(f, "  Sep_Out(THIS.");
	PTG_OUTPUT_STRING(f, n->p1);
	PTG_OUTPUT_STRING(f, "Ptg);\n");
	PTG_OUTPUT_STRING(f, "END;\n\n");
	...
	PTG_OUTPUT_STRING(f, "$/Output/Separator.fw\n");
	PTG_OUTPUT_STRING(f, "@}\n");
}
...
/* Implementation of Pattern Separator */

typedef struct _SPTGSeparator{
	_PTGProc _print;
	PTGNode p1;
} * _PPTGSeparator;

#ifdef PROTO_OK
static void _PrPTGSeparator(_PPTGSeparator n)
#else
static void _PrPTGSeparator(n)
	_PPTGSeparator n;
#endif
{
	n->p1->_print(n->p1);
	PTG_OUTPUT_STRING(f, " [Separator]");
}

#ifdef PROTO_OK
PTGNode PTGSeparator(PTGNode p1)
#else
PTGNode PTGSeparator(p1)

PTGNode p1;
#endif
{
	_PPTGSeparator n;
	n = (_PPTGSeparator)MALLOC(sizeof(struct _SPTGSeparator));
	n->_print = (_PTGProc)_PrPTGSeparator;
	n->p1 = p1;
	return (PTGNode)n;
}
